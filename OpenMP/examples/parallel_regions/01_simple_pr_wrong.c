 ...
 #pragma omp parallel               // this creates a parallel region
  ...
  {   
    ...
    my_thread_id = omp_get_thread_num();  // note: this assignment is not thread-safe
					  // my_thread_id is a shared variabled and its value
					  // is ovrwritten
    
    sleep(0.0005);                        // just to give all the threads enough time to
					  // complete the previous call. maybe it works, maybe not, though                                         
   
    nthreads = omp_get_num_threads();     // nthreads is being overwritten by every thread;
					  // pretty useless
